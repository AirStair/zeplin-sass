...
function joinRules(rules, layer) {
  count++
  html += '<div class="layer' + count + '">' + (layer.type === 'text' ? layer.content : '') + '</div>'
  html = html.replace(/\n/g, '')
  return '.layer' + count + '\n' + rules.join('\n') + '\n'
}
...
export default function layer(context, selectedLayer) {
    ...
    const setCode = (currentLayer, custom = '') => {
        ...
        code += declarationsToString(layerRuleSet.declarations, variableMap, textStyleMatch, currentLayer) + custom + '\n'
    }
